    ...
    <section id="column" className="py-12 md:py-24 bg-surface relative">
      <div className="max-w-[1200px] mx-auto px-4">
        <div className="flex flex-col md:flex-row items-end justify-between mb-8 md:mb-16 gap-6">
          <div>
            ...
            <p className="text-xl font-bold text-gray-600">
              日々の活動や、スタッフの想いを発信しています。
            </p>
          </div>
          <Link
            href="/column"
            className="hidden md:inline-block px-8 py-3 bg-white text-text border-3 border-border rounded-full font-black text-lg shadow-hard hover:shadow-none hover:translate-x-[2px] hover:translate-y-[2px] transition-all"
          >
            はなたね図書館に行く！
          </Link>
        </div>
          ...
          {columns.docs.map((col: any) => (
            ...
            >
              ...
              <div className="aspect-video w-full bg-gray-100 border-3 border-border mb-6 relative overflow-hidden rounded-lg">
                ...
                <span
                  className={`absolute top-3 right-3 px-3 py-1 rounded-full border-2 border-border font-black text-xs shadow-sm z-20 ${col.category === 'event' ? 'bg-pink text-text' : 'bg-lime text-text'}`}
                >
        ...
        <div className="text-center mt-12 md:hidden">
          <Link
            href="/column"
            className="inline-block px-10 py-4 bg-white text-text border-3 border-border rounded-full font-black text-xl shadow-hard hover:shadow-none hover:translate-x-[2px] hover:translate-y-[2px] transition-all"
          >
            VIEW ALL POSTS
          </Link>
        </div>
