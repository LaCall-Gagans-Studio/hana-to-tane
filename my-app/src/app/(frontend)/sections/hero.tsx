...
const MasonryImages = () => (
  <div className="grid grid-cols-2 gap-2 md:gap-4 p-2 md:p-8 transform -rotate-3 md:-rotate-6 scale-100 md:scale-110 origin-center md:origin-top-left pointer-events-none">
    <div className="space-y-2 md:space-y-4">
      <NextImage
        ...
        width={400}
        height={300}
        className="w-full rounded-2xl border-2 md:border-4 border-white shadow-md md:shadow-lg rotate-2"
      />
      <NextImage
        ...
        width={300}
        height={400}
        className="w-full rounded-2xl border-2 md:border-4 border-white shadow-md md:shadow-lg -rotate-1"
      />
    </div>
    <div className="space-y-2 md:space-y-4 pt-6 md:pt-12">
      <NextImage
  ...
  <div className="relative w-full h-full min-h-[500px] sm:min-h-[650px] pointer-events-none">
    ...
    >
      <div className="w-full h-full relative rounded-3xl overflow-hidden border-4 border-white shadow-xl">
        <NextImage
          src="https://images.unsplash.com/photo-1594608661623-aa0bd3a69d98?q=80&w=1548&auto=format&fit=crop"
          alt="Activity 1"
          fill
    ...
    >
      <div className="w-full h-full relative rounded-3xl overflow-hidden border-4 border-white shadow-2xl">
        <NextImage
          src="https://images.unsplash.com/photo-1621460248083-6271cc4437a8?q=80&w=3087&auto=format&fit=crop"
          alt="Activity 2"
          fill
          className="object-cover"
        />
...
export const Hero = ({ siteContent }: { siteContent?: any }) => {
  const largeLinks =
    siteContent?.heroLargeLinks?.length > 0
      ? siteContent.heroLargeLinks
      : [
          { label: 'はなとたねについて', url: '#about', colorClass: 'bg-[#cfed75]' },
          { label: 'イベントを見る', url: '#calendar', colorClass: 'bg-yellow' },
        ]
  const smallLinks =
    siteContent?.heroSmallLinks?.length > 0
      ? siteContent.heroSmallLinks
      : [
          { label: '認定フリースクールいっぽ', url: '/freeschool', hoverClass: 'hover:bg-blue' },
          { label: 'はなたね図書館', url: '/column', hoverClass: 'hover:bg-pink' },
          { label: 'サポート', url: '#support', hoverClass: 'hover:bg-orange-400' },
        ]
  ...
  return (
    <section className="bg-surface py-12 md:py-32 text-center border-b-4 border-border mb-16 md:mb-32 relative overflow-hidden">
      ...
      {/* Masonry Background Images (Left Side - PC Only) */}
      <div className="absolute z-10 bg-transparent top-0 left-0 w-1/2 h-[120%] overflow-hidden opacity-30 hidden xl:block">
        <MasonryImages />
      </div>
        ...
        <div className="flex flex-col xl:flex-row items-center justify-between gap-16 md:gap-12">
          ...
          <div className="flex-1 text-center xl:text-left relative w-full pt-4">
            ...
            <motion.h1
              ...
              className="text-[3rem] sm:text-[3.5rem] md:text-[5rem] lg:text-[7rem] font-black text-text mb-8 leading-[0.9] tracking-wide drop-shadow-xl"
            >
              <span className="block hover:scale-105 transition-transform duration-500 cursor-default">
                <span className="text-xl sm:text-2xl ml-2 text-surface mb-1 block">NPO法人</span>
                <span className="text-surface text-stroke-2 text-stroke-black drop-shadow-[4px_4px_0px_rgba(0,0,0,1)] block mb-10 md:mb-12">
                  はなとたね
                </span>
            ...
            <motion.div
              ...
              className="mb-10 text-left flex justify-center xl:justify-start"
            >
            ...
            <motion.div
              ...
              className="flex flex-col sm:flex-row gap-4 md:gap-6 justify-center xl:justify-start mt-4 mb-6"
            >
              {largeLinks.map((link: any, index: number) => (
                <Link
                  key={index}
                  href={link.url}
                  className={`group relative inline-block px-8 py-4 ${link.colorClass || 'bg-[#cfed75]'} text-text border-3 border-border rounded-2xl font-black text-lg md:text-xl shadow-hard transition-all hover:-translate-y-1 hover:shadow-hard-lg active:translate-y-0 active:shadow-hard`}
                >
            ...
            <motion.div
              ...
              className="flex flex-wrap gap-3 justify-center xl:justify-start text-sm md:text-base font-bold text-text mb-8 xl:mb-0"
            >
              {smallLinks.map((link: any, index: number) => (
                <Link
                  key={index}
                  href={link.url}
                  className={`px-4 py-2 bg-white border-2 border-border rounded-2xl ${link.hoverClass || 'hover:bg-blue'} hover:text-white transition-colors shadow-sm`}
                >
                  {link.label}
                </Link>
              ))}
